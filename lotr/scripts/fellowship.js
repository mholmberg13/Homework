...
const makeHobbits = () => {
  ...
  for (i = 0; i < hobbits.length; i++) {
      ...
      $listItem = $('<li>');
      $($listItem).append(hobbits[i]);
      $($listItem).attr('id', hobbits[i]);
...
const forgeTheFellowShip = () => {
  ...
  $fellow = $('<div id="the-fellowship">');
  // 2. add an h1 with the text 'The Fellowship' to this new div
  $($fellow).append('<h1>The Fellowship</h1>');
  // 3. append the fellowship to middle-earth
  $($section).append($fellow);
  // 4. add the unordered lists of hobbits and buddies to 'the-fellowship'
  $($fellow).append($buddies, $('#hobbits'));
};
...
const theBalrog = () => {
  ...
  buddies.splice(0, 1 , "Gandolf the White");
  $($buddies).remove();
  makeBuddies();
  
  // 2. add a class "the-white" to this element
  $('li[id="Gandolf the White"]').addClass('the-white')
